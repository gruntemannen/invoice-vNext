...
- `index.html`, `app.js`, `styles.css`: admin console with shared Cognito login, dashboard,
  invoice audit/detail, unified approvals, upload, NetSuite preview/transaction status, PDF
  download, delete, and NetSuite Test/Prod configuration.
...
- `src/api.ts`: JWT-protected HTTP API for stats, invoice list/detail/download/delete, upload,
  duplicate review decisions, unified approval queue, NetSuite runtime config, preview,
  durable transaction status, and replay.
- `src/extract.ts`: SQS extraction worker. Reads PDFs, invokes Claude Sonnet 4.6, normalizes the
  extraction, enriches VAT details, detects duplicates, assigns AP flow state, writes DynamoDB
  records, creates duplicate approvals, and ensures the automatic NetSuite outbox.
...
- `src/netsuite-worker.ts`: live NetSuite worker for idempotent invoice upserts, read-only vendor
  comparisons, and approved vendor-master PATCH operations.
...
- `src/shared/netsuite-outbox.ts`: deterministic automatic transaction creation, refresh, and
  queue orchestration.
- `src/shared/vendor-approvals.ts`: durable duplicate/vendor approval queue and state machine.
